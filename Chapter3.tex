...
% \subsection{Zero NACE results in no population transfer}
% As stated in section \ref{sec:calc_QM} there should be no population transfer when there is zero couping between electronic states. In the diabatic basis this means the diabatic NACE and the off-diagonal elements ($H_{ab}$) are both zero. This can be set in the code and should in the coefficients remaining constant. However, the BLAS subroutine I am using to determine the adiabatic-diabatic transformation matrix orders the Hamiltonian's eigenvectors (columns of the transformation matrix) by eigenvalue. This can be seen to result in an instantaneous adiabatic population transfer in the Marcus parabolas in figure \ref{fig:diab_wells}.
